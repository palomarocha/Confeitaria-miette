...
import { registerLocalUser, loginLocalUser, logoutLocalUser } from "./localAuth";
...
async function startServer() {
  ...
  app.post("/api/auth/login", loginLocalUser);
  app.post("/api/auth/logout", logoutLocalUser);
  app.use("/api/auth", (_req, res) => res.status(404).json({ error: "Auth endpoint not found" }));
